...
echo "Clearing .pytest_cache directories..."
...
if [ "$pyc_count" -eq 0 ] \
    && [ "$pycache_count" -eq 0 ] \
    && [ "$pytest_cache_count" -eq 0 ]; then

    echo "Done. Cache cleared."

else

    echo "Warning: $pyc_count .pyc file(s), $pycache_count __pycache__ dir(s), and $pytest_cache_count .pytest_cache dir(s) remain."
